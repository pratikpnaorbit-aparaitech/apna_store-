...
export default function RootNavigator() { const { user, loading } = useAuth(); const { ready, selectedAddress } = useAddress(); if (loading || !ready) return <SplashScreen />; return user ? <AppStack needsLocation={!selectedAddress} /> : <AuthStack />; }
