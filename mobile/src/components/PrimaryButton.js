...
export default function PrimaryButton({ title, onPress, loading, disabled, style, accessibilityLabel = title }) {
  ...
  return (
    <Animated.View style={[style, { transform: [{ scale }], opacity: disabled ? .55 : 1 }]}>
      <Pressable disabled={disabled || loading} onPress={onPress} onPressIn={pressIn} onPressOut={pressOut} accessibilityRole="button" accessibilityLabel={accessibilityLabel} accessibilityState={{ disabled: Boolean(disabled || loading), busy: Boolean(loading) }}>
